 ...
 */
 //将srcva地址对应的物理页面建立和dstva之间的映射关系，即把同一物理页面映射到不同的进程虚拟地址上
...
int sys_exofork(void) {
	...
	try(env_alloc(&e,curenv->env_id));//给子进程分配控制块
	/* Step 2: Copy the current Trapframe below 'KSTACKTOP' to the new env's 'env_tf'. */
	/* Exercise 4.9: Your code here. (2/4) */
	e->env_tf = *((struct Trapframe *)KSTACKTOP - 1);//保留目前父进程上下文到子进程
	/* Step 3: Set the new env's 'env_tf.regs[2]' to 0 to indicate the return value in child. */
	/* Exercise 4.9: Your code here. (3/4) */
	e->env_tf.regs[2]=0;//设置子进程返回值为0
	/* Step 4: Set up the new env's 'env_status' and 'env_pri'.  */
	/* Exercise 4.9: Your code here. (4/4) */
	e->env_status=ENV_NOT_RUNNABLE;
	e->env_pri=curenv->env_pri;
	return e->env_id;//父进程返回值为子进程的envid
}
...
void sys_exit_value(int value) {
	printk("exit_value:%d\n",value);
	curenv->env_exit_value = value;
}

int sys_wait_pid(int child){
	struct Env *e;
	e=&envs[ENVX(child)];
	if(e->env_status==ENV_FREE){
		printk("get_exit_value:%d\n",e->env_exit_value);
...
void sys_addjob(u_int envid, char *cmd) {
	struct Job *job;
	job = &jobs[curjob];
	job->job_env_id = envid;
	strcpy(job->job_cmd, cmd);
	job->job_id = curjob;
	curjob++;
}
...
void sys_printjob() {
	...
	struct Job *job;
	struct Env *env;
	for (int i = 1; i < curjob; i++) {
		job = &jobs[i];
		if (envid2env(job->job_env_id, &env, 0) == 0) {
			printk("[%d] %-10s 0x%08x %s\n", job->job_id, running, job->job_env_id, job->job_cmd);
		} else {
			printk("[%d] %-10s 0x%08x %s\n", job->job_id, done, job->job_env_id, job->job_cmd);
		}
	}
}

void sys_killjob(int jobid) {
	int envid = 0;
	struct Env *e;
	if (jobid < 1 || jobid >= curjob) {
		printk("fg: job (%d) do not exist\n", jobid);
		return;
	}
	struct Job * job = &jobs[jobid];
	envid = job->job_env_id;
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_exit_value] =sys_exit_value,
	[SYS_wait_pid]=sys_wait_pid,
	[SYS_addjob]=sys_addjob,
	[SYS_printjob]=sys_printjob,
	[SYS_killjob]=sys_killjob,
	[SYS_fgjob]=sys_fgjob,
};
 ...
 *   The possible arguments are stored at $a1, $a2, $a3, [$sp + 16 bytes], [$sp + 20 bytes] in order.
 *   Number of arguments cannot exceed 5.
 */
 //解析系统调用号、获取参数、调用相应的系统调用函数，并将返回值返回给用户程序
