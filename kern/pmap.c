...
void page_init(void) {
	...
	for(u_long i=0;i<used;i++){
	  pages[i].pp_ref=1;
	}

	/* Step 4: Mark the other memory as free. */
	for(u_long i=used;i<npage;i++){
	 pages[i].pp_ref=0;
 ...
 */
 //用于分配物理页面。将 page_free_list 空闲链表头部页控制块对应的物理页面分配出去，
 //将其从空闲链表中移除，并清空对应的物理页面，最后将 pp 指向的空间赋值为这个页控制块的地址。
...
// 给定一个虚拟地址，在给定的页目录（一级页表基地址）中查找这个虚拟地址对应的物理地址，
//如果这一虚拟地址对应的页目录项存在（也即页目录项 PTE_V 位为 1），则继续访问
// 页目录项对应二级页表，返回虚拟地址对应页表项的地址；如果虚拟地址对应的页目录项无效、不存在
// （也即页目录项 PTE_V 位为 0, 那么就不存在这一页目录项对应的二级页表），则根据传入的参数或创建二级页表，或返回空指针
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	/* Exercise 2.6: Your code here. (1/3) */
        pgdir_entryp=pgdir+PDX(va);//PDX获取虚拟地址的高12位作为一级页表项的地址偏移，相加获得一级页表项的地址，一级页表项指针指向二级页表的基地址。
	...
	if(!(*pgdir_entryp&PTE_V)){
	     if(create!=0){
		  ...
		  pp->pp_ref++; 
		  *pgdir_entryp=page2pa(pp)|PTE_C_CACHEABLE|PTE_V;//二级页表高20位表示物理页号+低12位权限标志=一级页表项
	     }else{
		     *ppte=NULL;
		     return 0;
	     }
	}
	//*ppte存的是二级页表项的地址，*pte存的是二级页表项的内容	
	/* Step 3: Assign the kernel virtual address of the page table entry to '*ppte'. */
	/* Exercise 2.6: Your code here. (3/3) */
	*ppte=(Pte *)KADDR(PTE_ADDR(*pgdir_entryp))+PTX(va);//二级页表项的地址，基地址（一级页表查到的物理页号）加虚拟地址的12-21位
	return 0;
}
 ...
 */
 ...
 //建立虚拟地址和对应物理页的映射关系，对页表和页目录进行重填，同时会清空tlb中的旧表项
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	/* Exercise 2.7: Your code here. (2/3) */
          try(pgdir_walk(pgdir,va,1,&pte)); // if(pgdir_walk(pgdir,va,1,&pte)!=0)return -E_NO_MEM;
