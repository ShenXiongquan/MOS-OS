...
void vprintfmt(fmt_callback_t out, void *data, const char *fmt, va_list ap) {
	...
	int width;	//输出宽度
	int long_flag; // output is long (rather than int)标记为long型
	int neg_flag;  // output is negative标记为负数
	int ladjust;   // output is left-aligned标记左对齐
	char padc;     // padding char填充多余位置所用的字符
