...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	while (secno < max) {
		...
		temp = 1;
		panic_on(syscall_write_dev(&temp, MALTA_IDE_NSECT, 1));//写入扇区数
		...
		panic_on(syscall_write_dev(&temp, MALTA_IDE_DEVICE, 1));//将扇区编号依次写入寄存器
		...
		for (int i = 0; i < SECT_SIZE / 4; i++) {
			panic_on(syscall_read_dev(dst + offset + i * 4, MALTA_IDE_DATA, 4));//读取数据，每次读取4byte
		}
