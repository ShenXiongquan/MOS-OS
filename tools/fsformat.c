...
struct Block {
	...
	uint32_t type;
} disk[NBLOCK];//disk被分为多个磁盘块，其中的每个盘
...
struct File *create_file(struct File *dirf) {
	int nblk = dirf->f_size / BLOCK_SIZE;//目录中总共的磁盘块数

	// Step 1: Iterate through all existing blocks in the directory.
	for (int i = 0; i < nblk; ++i) {
		int bno; // the block number,磁盘块号
		...
		if(i<NDIRECT){
			bno=dirf->f_direct[i];//f_direct[i]是指向磁盘块的指针，里面存的是磁盘块号
		}else{
			bno=((uint32_t *)(disk[dirf->f_indirect].data))[i];//用间接索引先取出磁盘控制块，再从数据中取出磁盘块号
		}
