...
struct Trapframe {
	...
	unsigned long regs[32];//cpu的寄存器

	/* Saved special registers. */
	unsigned long cp0_status;//
	unsigned long hi;//
	unsigned long lo;//
	unsigned long cp0_badvaddr;//保存最近发生地址异常时的异常地址
	unsigned long cp0_cause;//保存中断和异常的原因
	unsigned long cp0_epc;//保存最近一次发生异常时的程序计数器（PC）的值，即异常发生时的指令地址
};
