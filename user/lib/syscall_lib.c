...
void syscall_exit_value(int value){
	msyscall(SYS_exit_value,value);
}
int syscall_wait_pid(int child){
	return msyscall(SYS_wait_pid,child);
}


void syscall_addjob(u_int envid, char *cmd) {
	msyscall(SYS_addjob, envid, cmd);
}

void syscall_printjob() {
	msyscall(SYS_printjob);
}

void syscall_killjob(int jobid) {
	msyscall(SYS_killjob, jobid);
}

int syscall_fgjob(int jobid) {
	return msyscall(SYS_fgjob, jobid);
}